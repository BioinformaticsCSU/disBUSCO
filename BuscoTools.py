...
class AugustusRunner:
    ...
    def generate_jobs(self):
        ...
        if os.path.exists(os.getcwd()+"/buscohosts.txt"):
            with open(os.getcwd()+"/buscohosts.txt",'r') as f:
                    hosts=''.join(f.readlines()).strip().split('\n')
            nprocs=len(hosts)
            print("use buscohosts.txt to get nprocs")
        else:
            nprocs=35

#        with open('/public/home/hpc/zouyou/bio/busco/busco-test/Cyprinodon/tmp.txt','wb') as f:
#            pickle.dump(list(self.coords.items()),f)
        
        bus_tmp=list(self.coords.items())
        li=[]
        ind=0
        for busco_group, contigs in self.coords.items():
            tmp_contigs=list(contigs.items())
            li.append((ind,tmp_contigs[0][1]['contig_end']-tmp_contigs[0][1]['contig_start']))
            ind=ind+1
        li=sorted(li,key=lambda s:s[1],reverse=True)
        coords={}
        for i in range(0,len(bus_tmp)):
            coords[bus_tmp[li[i][0]][0]]=bus_tmp[li[i][0]][1]
        self.coords=coords
####end of sort augutus task
...
class ETrainingRunner:
    ...
    def __init__(self, etraining_tool, main_out, output_folder, cpus, augustus_config_path):
        self.etraining_tool = etraining_tool
        self.new_species_name = "BUSCO_{}".format(os.path.basename(main_out))
        self.output_folder = output_folder
        self.cpus = cpus
        self.augustus_config_path = augustus_config_path
