...
class Tool:
    ...
    def run_jobs(self, max_threads):
        ...
        if os.path.exists(os.getcwd()+"/buscohosts.txt") and self.jobs_to_run[0].cmd_line[0][-11:]=="augustusdis":
            print("using buscohosts.txt to run augustus")
            hostfile=os.getcwd()+"/buscohosts.txt"
            with open(hostfile,'r') as f:
                hosts=''.join(f.readlines()).strip().split('\n')
            nprocs=len(hosts)
            print("num_hosts=",len(hosts)) 
            while len(self.jobs_to_run) > 0 or len(self.jobs_running) > 0:
                time.sleep(0.001)
                for j in self.jobs_to_run:
                    if len(self.jobs_running) < nprocs:
                        self.jobs_running.append(j)
                        self.jobs_to_run.remove(j)
                        runhost=hosts.pop()
                        j.cmd_line.append("-host")
                        j.cmd_line.append(runhost)
                        print(" ".join(j.cmd_line),"len_hosts:",len(hosts),"jobs_to_run:",len(self.jobs_to_run),"jobs_running:",len(self.jobs_running))
                        j.start()
            ...
            print(self.jobs_to_run[0].cmd_line)
            while len(self.jobs_to_run) > 0 or len(self.jobs_running) > 0:
                time.sleep(0.001)
                for j in self.jobs_to_run:
    ...
    @log(time.asctime(time.localtime(time.time()))+' [{0}]\t{1} of {2} task(s) completed', logger, attr_name=['name', 'nb_done', 'total'], on_func_exit=True)
...
#                    print(j.cmd_line)
#                    j.cmd_line[0]="/public/home/hpc/zouyou/.conda/envs/busco406/bin/tblastn"
#                    print(j.cmd_line)
#                    print(os.getcwd())
#                    j.start()
#            for j in self.jobs_running:
